...
static bool 句柄不可用(const SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX* 系统句柄表条目信息头, UniqueHandle& SourceProcessHandle, UniqueHandle& TargetHandle)
{
	...
	if (!(SourceProcessHandle = OpenProcess(PROCESS_DUP_HANDLE, FALSE, 系统句柄表条目信息头->UniqueProcessId)))
	{
		...
		case ERROR_INVALID_PARAMETER://进程已关闭
...
static wchar_t const* 查询串口设备路径(const std::wstring& COM)noexcept
{
	static HKEY SERIALCOMM = nullptr;
	if (!SERIALCOMM)
		RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"HARDWARE\\DEVICEMAP\\SERIALCOMM", 0, KEY_READ, &SERIALCOMM);
	if (SERIALCOMM)
	{
...
API(Serialport_snatch)
{
	...
	wchar_t const* const 串口设备路径 = 查询串口设备路径(COM);
	if (串口设备路径)
	{
...
API(IO_FindLocking)
{
	const std::wstring 输入路径 = 读入UTF16字符串();
	wchar_t const* const 串口设备路径 = 查询串口设备路径(输入路径);
	DWORD const 文件属性 = GetFileAttributesW(输入路径.c_str());
	bool 查找文件 = 文件属性 != INVALID_FILE_ATTRIBUTES;
	bool 目标是目录;
	static std::wstring 目标路径;
	if (查找文件)
	{
		目标是目录 = 文件属性 & FILE_ATTRIBUTE_DIRECTORY;
		const HANDLE FileHandle = CreateFileW(输入路径.c_str(), FILE_READ_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, nullptr, OPEN_EXISTING, 目标是目录 ? FILE_FLAG_BACKUP_SEMANTICS : FILE_ATTRIBUTE_NORMAL, nullptr);
	...
	if (!(查找文件 || 串口设备路径))
	{
		...
		case ERROR_PATH_NOT_FOUND:
			throw MATLAB::Exception::File_not_found;
		default:
			throw 有头Win32异常{ MATLAB::Exception::Failed_to_open_the_file, LastError };
		}
	}
	const SYSTEM_HANDLE_INFORMATION_EX* const 系统句柄信息 = 获取系统句柄信息();
	const SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX* 系统句柄表条目信息头 = 系统句柄信息->Handles;
	...
	for (;;)
	{
		...
		for (;;)
		{
			if (DeviceIoControl(获取ProExp152(), 0x83350048, &InBuffer, sizeof(InBuffer), OutBuffer.get(), OutBuffer.size(), nullptr, nullptr))
			{
				wchar_t const* const 当前路径 = (reinterpret_cast<ProcExp_OutBuffer*>(OutBuffer.get()))->文件名();
				bool 匹配 = false;
				if (查找文件)
				{
					const int 目标长度 = static_cast<int>(目标路径.size());
					const wchar_t 后续字符 = 当前路径[目标长度];
					匹配 = CompareStringOrdinal(目标路径.c_str(), 目标长度, 当前路径, 目标长度, TRUE) == CSTR_EQUAL && (!后续字符 || 目标是目录 && 后续字符 == L'\\');
				}
