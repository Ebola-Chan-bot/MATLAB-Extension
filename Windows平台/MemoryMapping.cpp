...
API声明(MemoryMapping_Create)
{
	...
	size_t 总字节数; 
	CellArray 输入元胞 = 元胞字节化(std::move(inputs[5]), 总字节数);
...
API声明(MemoryMapping_Open)
{
	...
	const DWORD DesiredAccess = 万能转码<DWORD>(std::move(inputs[1]));
...
API声明(MemoryMapping_View)
{
	...
	const LPVOID 映射指针 = MapViewOfFile(万能转码<HANDLE>(std::move(inputs[1])), 万能转码<DWORD>(std::move(inputs[2])), FileOffset.HighPart, FileOffset.LowPart, 万能转码<SIZE_T>(std::move(inputs[4])));
	if (映射指针)
		outputs[1] = 万能转码(映射指针);
	else
...
{
	CloseHandle(万能转码<HANDLE>(std::move(inputs[1])));
}