%[text] 调用 Win32 ShellAPI 执行可撤销的批量文件复制
%[text] 不同于内置copyfile，本函数执行后可以在Windows文件资源管理器上下文菜单中撤销操作（被覆盖的文件无法恢复）。此外，源和目标都可以是字符串数组，以便执行批量复制操作，不需要具有相似的文件名模式。通配符也同样支持。
%[text] 操作过程中出现文件已存在、目录不存在等异常，都可以手动确认操作，与Windows文件资源管理器深度整合。
%[text] 不支持内置copyfile的f旗帜。
%[text] ## 输入参数
...
%[text] **See also** [copyfile](<matlab:doc copyfile>)
function AnyOperationCanceled = CopyFile(Source,Destination)
AnyOperationCanceled=MATLAB.internal.WindowsAPI.SHFile_Copy(Source,Destination);
