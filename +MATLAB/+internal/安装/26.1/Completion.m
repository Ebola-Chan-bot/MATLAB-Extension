classdef Completion < handle
	...
	properties (Access=private)
		...
		% Products found by VER
		nameOfProductsOnPath
	...
	methods (Access = private)
			...
			if ~isempty(obj.ScanList)
				...
				while ~isempty(obj.ScanList)
					...
					while ~isempty(addedFiles)
						...
						sym = resolveRootSet(obj, files, ignoreFileList);
		...
		function pid = productsShippingComponent(obj, components)
			...
			pinfo = obj.pcm_navigator.productShippingComponentWithLicenseTest(components, obj.Target, test_license, obj.nameOfProductsOnPath);
			pid = cell(size(pinfo));
			for k = 1:numel(pinfo)
				if ~isempty(pinfo{k})
		...
		function recordPackageDependency(obj, fpath, pkgList)
			if isKey(obj.fileToPackages, fpath)
				l = obj.fileToPackages(fpath);
				obj.fileToPackages(fpath) = union(l, pkgList);
			else
				obj.fileToPackages(fpath) = pkgList;
			end
		end
		...
		function symbols = resolveRootSet(obj, fileList, ignorableFiles)
			...
			for k = 1:num_file
				...
				newSymbol = inspector.determineType(name);
		...
		function rootSymbols = initializeRootSet(obj, files)
			...
			rootSymbols = resolveRootSet(obj, files, ignoreFileList);
		...
		function list = filterNonNamespacedFcnAgainstDetectedClsList(~, list)
			...
			mList = ignorableMethods(matlab.depfun.internal.MatlabSymbol.classList);
			if ~isempty(mList)
				ignore_idx = ismember(list, mList);
			end
			if any(ignore_idx)
				list(ignore_idx) = [];
			end
			...
			for k = 1:numel(list)
				...
				if ~ismember(cname, matlab.depfun.internal.requirementsConstants.componentsInMATLAB)
					pinfo = obj.pcm_navigator.productShippingComponentWithLicenseTest(cname, ...
						matlab.depfun.internal.Target.MATLAB, test_license, obj.nameOfProductsOnPath);
					pinfo = pinfo{1};
					if isempty(pinfo)
						keep(k) = false;
					end
	...
	methods
		...
		function obj = Completion(varargin)
			...
			% VER is incredibly expensive. Only call it once here.
			productsOnPath = ver();
			obj.nameOfProductsOnPath = {productsOnPath.Name}';
		...
		function unresolved = unresolvedSymbols(obj)
			...
			for i = 1:numel(inspectorName)
				...
				for k = 1:numel(files)
					unresolved_list = UnresolvedSymbolMap(files{k});
					% Ignore class methods
					reduced_list = obj.filterNonNamespacedFcnAgainstDetectedClsList(unresolved_list);

					% find out which symbol is builtin from undeployable
					keep = ismember(unresolved_list, matlab.depfun.internal.requirementsConstants.undeployableBuiltins);
					builtinFromUndeployable = unresolved_list(keep);

					% keep the unresolvable & builtin from undeployable
					UnresolvedSymbolMap(files{k}) = union(reduced_list, builtinFromUndeployable);
				end
				unresolved = [unresolved; obj.Inspectors(inspectorName{i}).UnresolvedSymbols]; %#ok
			end
		end
		...
		function [deployable, reason] = isdeployable(obj, files, entryPoint)
			...
			ignoreFileList = {};
			symbols = resolveRootSet(obj, files, ignoreFileList);
