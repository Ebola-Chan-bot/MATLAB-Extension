classdef (Hidden, Sealed) BuildRunner < ...
	...
	properties (Access = {?matlab.buildtool.internal.BuildRunnerExtension})
		BuildRunStrategy
	end

	properties (Access = {?matlab.buildtool.internal.BuildRunStrategy})
	...
	methods (Hidden)
		function result = runCore(runner, plan, taskName, taskArgs, options)
			...
			import matlab.buildtool.plugins.plugindata.BuildRunPluginData;
			...
			runner.PluginData.runBuild = BuildRunPluginData("");

			runner.evaluateMethodOnPlugins("runBuild", runner.PluginData.runBuild);
			result = runner.PluginData.runBuild.BuildResult;
			delete(runner.PluginData.runBuild);
		end
	...
	methods (Access = ?matlab.buildtool.internal.BuildRunStrategy)
	...
	methods (Access = private)
		function runner = BuildRunner()
			...
			import matlab.buildtool.internal.SerialBuildRunStrategy;

			runner.Fingerprinters = Fingerprinter.default();
			runner.OperatorList = BuildContentOperatorList(runner);
			runner.BuildRunStrategy = SerialBuildRunStrategy();
		end
	...
	methods (Hidden, Access = protected)
		function runBuild(runner, pluginData)
			runner.beginPluginMethod("runBuild", pluginData);
			runner.BuildRunStrategy.runBuild(runner, pluginData);
			runner.completePluginMethod();
		end

		function scheduleTaskGraph(runner, pluginData)
			runner.beginPluginMethod("scheduleTaskGraph", pluginData);

			[result, idx] = runner.BuildRunData.getTaskResult(pluginData.Name);
			result.Started = true;
			runner.BuildRunData.updateResults(result);

			runner.BuildRunStrategy.scheduleTask(runner, pluginData.WorkerId, idx);

			runner.completePluginMethod();
		end

		function completeTaskGraph(runner, pluginData)
			runner.beginPluginMethod("completeTaskGraph", pluginData);
			runner.BuildRunData.updateResults(pluginData.TaskResults);
			runner.completePluginMethod();
		end

		function runTaskGraph(runner, pluginData)
			import matlab.buildtool.internal.TaskIterator;
			import matlab.buildtool.fingerprints.TaskChanges;
			import matlab.buildtool.fingerprints.FingerprintContext;
			import matlab.buildtool.diagnostics.TaskRunReason;
			import matlab.buildtool.diagnostics.TaskSkipReason;
			...
			% Create iterator
			iterator = TaskIterator(pluginData.TaskGraph);
			...
			if runner.FixtureErrored && ~runner.BuildRunData.ContinueOnFailure
				runner.skipTasks(iterator.remainingTasks(), TaskSkipReason.DependencyFailed);
			end
			...
			buildOptions = struct( ...
				...
				"UI", runner.BuildRunData.UI);

			while iterator.hasTasksRemaining()
				task = iterator.selectNextTask();
				...
				elseif shouldTrack
					...
					if ~isempty(previousTrace)
						currentTrace = computeTaskTrace(task, runner.BuildRunData.getTaskArgs(task.Name), fpArgs{:});
						...
						taskChanges = TaskChanges.withAllPropertiesAdded(task, runner.BuildRunData.getTaskArgs(task.Name), fpArgs{:});
				...
				% Run or skip task
				[~, idx] = runner.BuildRunData.getTaskResult(task.Name);
				...
				if runner.BuildRunData.getTaskResult(task.Name).Failed && ~runner.BuildRunData.ContinueOnFailure
					runner.skipTasks(iterator.remainingTasks(), TaskSkipReason.DependencyFailed);
				elseif shouldTrack && ~runner.BuildRunData.getTaskResult(task.Name).Skipped
					newTrace = computeTaskTrace(task, runner.BuildRunData.getTaskArgs(task.Name), fpArgs{:});
		...
		function context = createTaskContext(runner, pluginData)
			...
			task = runner.BuildRunData.Plan(pluginData.Name);
			context = TaskContext(task, runner.BuildRunData.Plan, ...
				TaskChanges=pluginData.TaskChanges, ...
				BuildOptions=pluginData.BuildOptions);

			context.addPostFailureEventCallback(@(info)runner.recordTaskFailure(task.Name, info.Type));
		...
		function runTask(runner, pluginData)
			...
			task = runner.BuildRunData.Plan(pluginData.Name);
			[result , idx] = runner.BuildRunData.getTaskResult(task.Name);
			result.Started = true;
			...
			result.ValidationFailed = ~isempty(failures);
			runner.BuildRunData.updateResults(result);
			...
			for action = task.Actions
				if runner.BuildRunData.getTaskResult(task.Name).Failed
					break;
				end
				...
				runner.PluginData.runTaskAction = TaskActionRunPluginData( ...
					...
					idx, ...
					action, ...
					runner.BuildRunData.getTaskArgs(task.Name));
			...
			if ~runner.BuildRunData.getTaskResult(task.Name).Failed
				...
				result = runner.BuildRunData.getTaskResult(task.Name);
				result.ValidationFailed = ~isempty(failures);
				runner.BuildRunData.updateResults(result);
			end
		...
		function runTaskAction(runner, pluginData)
			...
			try
				t0 = tic();
				args = runner.BuildRunData.getTaskArgs(pluginData.Task.Name);
				action.evaluate(runner.CurrentTaskContext, args{:});
				duration = seconds(toc(t0));
				runner.BuildRunData.addDurationToTaskResult(pluginData.Task.Name, duration);
			catch exception
				duration = seconds(toc(t0));
				runner.BuildRunData.addDurationToTaskResult(pluginData.Task.Name, duration);
				if ~(metaclass(exception) < ?matlab.buildtool.internal.qualifications.QualificationFailedException)
					...
					runner.recordTaskFailure(pluginData.Task.Name, "Errored");
				end
			end
			runner.completePluginMethod();
		end
		...
		function skipTask(runner, pluginData)
			...
			result = runner.BuildRunData.getTaskResult(pluginData.Name);
			result.Description = pluginData.Plan(pluginData.Name).Description;
			result.Skipped = true;
			result.SkipReason = pluginData.SkipReason;
			runner.BuildRunData.updateResults(result);
			runner.completePluginMethod();
		end
	end
	...
	methods (Access = ?matlab.buildtool.internal.BuildRunStrategy)
		function skipTasks(runner, tasks, reason)
			arguments
				runner (1,1) matlab.buildtool.BuildRunner
				tasks (1,:) matlab.buildtool.Task
				reason (1,1) matlab.buildtool.diagnostics.TaskSkipReason
			end
	...
	methods (Access = private)
		...
		function recordTaskFailure(runner, taskName, property)
			result = runner.BuildRunData.getTaskResult(taskName);
