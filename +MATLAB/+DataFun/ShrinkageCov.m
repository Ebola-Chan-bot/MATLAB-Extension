...
SC=zeros(FeatSize,FeatSize,OtherSize);
...
if anymissing(Tensor)
	...
	% 目标矩阵：缩放单位阵
	mu=squeeze(sum(S.*reshape(eye(FeatSize),FeatSize,FeatSize,1),[1,2]))./FeatSize; % [page,1]
	F=reshape(mu,1,1,[]).*reshape(eye(FeatSize),FeatSize,FeatSize,1);
